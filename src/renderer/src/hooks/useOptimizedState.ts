...
export const useStyles = () => {
  return useMemo(() => ({
    ...
    dropOverlay: {
      className: 'fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50 pointer-events-none'
    },
    mainContainer: {
      className: 'flex flex-col h-screen pt-10 bg-background text-text-primary overflow-hidden transition-colors duration-200'
    },
    toolsModalOverlay: {
      className: 'fixed inset-0 bg-black bg-opacity-50 dark:bg-opacity-70 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in'
    },
    toolsModalContent: {
      className: 'bg-surface rounded-2xl p-8 max-w-md w-full mx-4 shadow-xl dark:shadow-dark-xl animate-slide-down'
    },
    progressBar: {
...
export const useClassNames = () => {
  ...
  const getProgressBarFillStyle = useCallback((progress: number) => ({
    width: `${progress}%`
  }), [])
